...
- POS: immutable `Sale`/`SaleItem` snapshots, FEFO `SaleItemBatch` allocations, `Payment` records, and `Refund`/refund-batch records. `Register` and `CashShift` provide location-scoped cash accountability with opening cash, expected/counting/variance values, guarded open/close transitions, and audit events. Sale completion, stock deduction, payments, and audit events share one transaction; sales can be linked to an open shift.
